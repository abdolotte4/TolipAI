...
function NotificationBell() {
  ...
  useEffect(() => { if (prevUnread.current === null) { prevUnread.current = unread; return; } if (unread > prevUnread.current) { const newCount = unread - prevUnread.current; playNotificationSound(); if ("Notification" in window && Notification.permission === "granted") new Notification("Digor CRM", { body: newCount === 1 ? "You have 1 new notification." : `You have ${newCount} new notifications.`, icon: "/favicon.ico", tag: "crm-notification" }); } prevUnread.current = unread; }, [unread]);
  useEffect(() => { const base = "Digor CRM"; document.title = unread > 0 ? `(${unread}) ${base}` : base; return () => { document.title = base; }; }, [unread]);
...
export function AppLayout({ children }: AppLayoutProps) {
  ...
  const mainNavItems = isSuperAdmin ? [{ label: "Campaigns", href: "/campaigns", icon: Globe }, { label: "All Leads", href: "/leads", icon: Users, badge: newLeadsCount }, { label: "Pipeline", href: "/pipeline", icon: Columns }, { label: "All Tasks", href: "/tasks", icon: CheckSquare, badge: pendingTasksCount }, { label: "Buyers List", href: "/buyers", icon: BookUser }, { label: "Cash Buyer DB", href: "/cash-buyers", icon: Database }, { label: "Distressed Lead Gen", href: "/lead-gen", icon: Zap }] : [{ label: "Dashboard", href: "/", icon: LayoutDashboard }, { label: "Leads", href: "/leads", icon: Users, badge: newLeadsCount }, { label: "Pipeline", href: "/pipeline", icon: Columns }, { label: "Tasks", href: "/tasks", icon: CheckSquare, badge: pendingTasksCount }, { label: "Buyers List", href: "/buyers", icon: BookUser }, { label: "Cash Buyer DB", href: "/cash-buyers", icon: Database }, { label: "Distressed Lead Gen", href: "/lead-gen", icon: Zap }];
  const adminNavItems = [{ label: "Team Users", href: "/admin/users", icon: Settings }, { label: "Submission Links", href: "/admin/links", icon: LinkIcon }, { label: "Email Sequences", href: "/admin/sequences", icon: Mail }];
