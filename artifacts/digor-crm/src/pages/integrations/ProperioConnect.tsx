...
export default function ProperioConnect() {
  ...
  const invalidateMutation = useMutation({
    mutationFn: () =>
      apiFetch("/scraper-engine/integrations/propelio/session", { method: "DELETE" }),
    onSuccess: () => {
  ...
  async function handleTest() {
    ...
    try {
      ...
      if (result.success) {
        toast({ title: "Login successful!", description: "Propelio credentials are working correctly." });
      } else {
    ...
    <div className="space-y-6 pb-20 max-w-2xl">
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <a
            href="https://genesis.propelio.com"
            target="_blank"
      ...
      <Card className="rounded-2xl border-white/5 bg-card p-5">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium text-sm">
                {isLoading ? "Checking status…" : configured ? "Credentials configured" : "Not configured"}
              </p>
              {configured && emailMasked && (
                <p className="text-xs text-muted-foreground mt-0.5">Account: {emailMasked}</p>
              )}
            </div>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            {configured && (
              ...
              >
                {sessionActive ? (
                  ...
                  <><WifiOff className="w-3 h-3 mr-1" /> No session</>
                )}
              </Badge>
            )}
            {configured && (
              <Button
                variant="outline"
                size="sm"
                className="rounded-xl gap-1.5 border-white/10 text-xs h-7"
                onClick={handleTest}
                disabled={testing}
              >
            ...
            {configured && sessionActive && (
              <Button
                variant="outline"
                size="sm"
                className="rounded-xl gap-1.5 border-white/10 text-xs h-7 hover:bg-amber-500/10 hover:border-amber-500/20 hover:text-amber-400"
      ...
      {isAdmin && (
        <Card className="rounded-2xl border-white/5 bg-card p-6 space-y-5">
          ...
          <p className="text-xs text-muted-foreground">
            Enter your Propelio email and password. They are stored AES-256 encrypted and used only by the scraper engine to pull cash buyers and comps for your campaign.
          </p>
          ...
          <Button
            ...
            onClick={() => saveMutation.mutate({ email: email.trim(), password: password.trim() })}
          >
          ...
          {configured && (
            <div className="pt-1 border-t border-white/5">
              <button
                className="text-xs text-destructive/70 hover:text-destructive flex items-center gap-1.5 mt-3"
                onClick={() => {
                  if (confirm("Remove Propelio credentials for this campaign?")) {
      ...
      {/* Info */}
      <Card className="rounded-2xl border-white/5 bg-card/50 p-5">
        <h3 className="text-xs font-semibold uppercase text-muted-foreground tracking-wider mb-3">What Propelio powers</h3>
        <ul className="space-y-2 text-sm text-muted-foreground">
          {[
            "Cash buyer lists — the \"5523 results\" panel with landlord/flipper filters",
            "MLS comparable sales for ARV estimation",
            "Owner skip-trace (when your subscription includes it)",
          ].map((item) => (
