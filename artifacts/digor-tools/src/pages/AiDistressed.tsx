...
export default function AiDistressed() {
  ...
  useEffect(() => {
    if (!jobId) return;
    if (job?.status === "completed" || job?.status === "failed") { if (pollRef.current) clearInterval(pollRef.current); return; }
