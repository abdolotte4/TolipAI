...
## Troubleshooting — IAM Trust Policy Error

**Symptom:** ECS fails to launch tasks with:
```
ECS was unable to assume the role 'arn:aws:iam::ACCOUNT_ID:role/tolipai-scraper-task-role'
...
**Cause:** The IAM task role or execution role does not have `ecs-tasks.amazonaws.com` as a trusted principal. This happens when the role was created via the AWS Console instead of the CLI steps in this guide.

**Fix:**
```bash
for ROLE in tolipai-scraper-task-role tolipai-scraper-execution-role; do
  ...
  echo "Updated trust policy for $ROLE"
done
```

## Troubleshooting — `ACCOUNT_ID` Placeholder in Task Definition

`ecs-task-definition.json` ships with `ACCOUNT_ID` as a placeholder. `deploy.sh` substitutes your real account ID automatically via:
```bash
sed "s|ACCOUNT_ID|${AWS_ACCOUNT_ID}|g" ecs-task-definition.json | aws ecs register-task-definition ...
```
Always run deployments through `deploy.sh`, not by registering `ecs-task-definition.json` directly.

---

*Last updated: May 23, 2026 — TolipAI Platform v2.1.0*
