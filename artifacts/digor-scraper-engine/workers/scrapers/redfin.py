...
async def _gis_csv(region_id: str, region_type: int, *,
                   sold: bool = True,
                   max_results: int = 100,
                   market: str = "na",
                   sold_within_days: int = 180) -> List[Dict[str, Any]]:
...
async def fetch_recently_sold(zip_code: Optional[str] = None,
    ...
    rows = await _gis_csv(region["region_id"], region["region_type"],
                          ...
                          market=market, sold_within_days=sold_within_days)
    ...
    for r in rows:
        ...
        try:
            price_cut = (
                float(str(price_str).replace("$", "").replace(",", "") or 0)
        ...
        out.append({
            ...
            "redfin_url": r.get("URL") or r.get("URL (SEE https://www.redfin.com/buy-a-home/comparative-market-analysis FOR INFO ON PRICING)"),
            "latitude": lat,
            "longitude": lon,
        })
    return out
