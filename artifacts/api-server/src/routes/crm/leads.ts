...
router.post("/", crmAuth, async (req, res) => {
  ...
  try {
    ...
    const [lead] = await db.insert(crmLeads).values({
      ...
      skipTracedName: data.skipTracedName || null,
      skipTracedPhones: Array.isArray(data.skipTracedPhones) ? data.skipTracedPhones : null,
      skipTracedEmails: Array.isArray(data.skipTracedEmails) ? data.skipTracedEmails : null,
