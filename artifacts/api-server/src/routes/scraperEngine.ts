...
async function _buyerLeadIds(user: CrmTokenPayload): Promise<number[] | null> {
  ...
  const rows = await db.select({ id: crmLeads.id }).from(crmLeads).where(eq(crmLeads.campaignId, user.campaignId));
  return rows.map(r => r.id);
}

function _buyerWhere(scopeIds: number[] | null, q: Record<string, string>): SQL | undefined {
  const conds: SQL[] = [];
  if (scopeIds !== null) {
  ...
  if (q["minPortfolioSize"]) conds.push(gte(cashBuyerMatches.portfolioSize, Number(q["minPortfolioSize"])));
  if (q["maxPortfolioSize"]) conds.push(lte(cashBuyerMatches.portfolioSize, Number(q["maxPortfolioSize"])));
...
router.get("/scraper-engine/buyers/export.csv", crmAuth, async (req: Request, res: Response) => {
  try {
    ...
    const rows = await db
      .select({
        id: cashBuyerMatches.id, leadId: cashBuyerMatches.leadId,
        buyerName: cashBuyerMatches.buyerName, llcName: cashBuyerMatches.llcName,
        buyerType: cashBuyerMatches.buyerType, matchScore: cashBuyerMatches.matchScore,
        source: cashBuyerMatches.source, city: cashBuyerMatches.city,
        state: cashBuyerMatches.state, zip: cashBuyerMatches.zip,
        ...
        phones: cashBuyerMatches.phones, emails: cashBuyerMatches.emails,
        createdAt: cashBuyerMatches.createdAt, leadAddress: crmLeads.address,
      })
    ...
    const header = "id,leadId,buyerName,llcName,buyerType,matchScore,source,city,state,zip,mailingAddress,portfolioSize,portfolioValue,avgPurchasePrice,lastPurchaseDate,phones,emails,createdAt,leadAddress";
    const lines = rows.map(r =>
      [r.id, r.leadId, esc(r.buyerName), esc(r.llcName), r.buyerType, r.matchScore,
       r.source, r.city ?? "", r.state ?? "", r.zip ?? "", esc(r.mailingAddress),
       r.portfolioSize ?? "", r.portfolioValue ?? "", r.avgPurchasePrice ?? "",
       r.lastPurchaseDate ?? "",
       esc((r.phones as string[]).join(";")),
       esc((r.emails as string[]).join(";")),
       r.createdAt.toISOString(), esc(r.leadAddress)].join(",")
    );
    res.setHeader("Content-Type", "text/csv");
    res.setHeader("Content-Disposition", `attachment; filename="cash-buyers-${new Date().toISOString().slice(0, 10)}.csv"`);
  ...
  try {
    ...
    const rows = await db
      .select({
        ...
        avgPurchasePrice: cashBuyerMatches.avgPurchasePrice,
        lastPurchaseDate: cashBuyerMatches.lastPurchaseDate,
        city: cashBuyerMatches.city, state: cashBuyerMatches.state, zip: cashBuyerMatches.zip,
        mailingAddress: cashBuyerMatches.mailingAddress,
        phones: cashBuyerMatches.phones, emails: cashBuyerMatches.emails,
