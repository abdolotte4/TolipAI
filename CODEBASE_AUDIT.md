...
| S23-j | **STOP confirmation SMS** — STOP and HELP handlers in `artifacts/api-server/src/routes/twilio.ts` moved from inside `setImmediate` (gated on `aiSmsEnabled`) to top-level webhook handler (unconditional). STOP now: (1) writes to `crmSmsOptOuts` immediately, (2) fires `sendSms` confirmation "You've been unsubscribed…" via fire-and-forget `setImmediate`. HELP likewise sends carrier-required support text unconditionally. Both short-circuit with `return` before notifications and AI SMS. |
| S23-k | **Fargate migration guide fixes** — `infrastructure/FARGATE_MIGRATION.md` updated throughout to match actual JSON/script files: port 8000→**8765**, ECR repo `tolipai/scraper-engine`→`TolipAI-scraper`, cluster `tolipai-scraper`→`TolipAI-scraper-cluster`, container name `scraper-engine`→`scraper`, IAM roles `TolipaiScraper*`→`TolipAI-scraper-*-role`, task file `task-definition.json`→`ecs-task-definition.json`, secrets prefix `/tolipai/scraper/`→`TolipAI/scraper/`, CPU/memory 1024/3072→2048/4096, Python 3.12→3.11, private subnets added. All 14 secrets listed. "Resource Name Reference" table added. "Quick Deploy" section added referencing `deploy.sh`. |
