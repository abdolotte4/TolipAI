...
def _extract_contacts(html: str, source: str) -> Dict[str, Any]:
    ...
    for m in _NAME_RE.finditer(text[:3000]):
        ...
        if (len(name) > 5 and name not in seen_names
                and not any(w in name for w in ("Search", "Result", "People", "Address",
                                                "Phone", "Email", "Background", "Check",
                                                "United", "States", "Street"))):
            seen_names.add(name)
            names.append(name)

    return {"phones": phones, "emails": emails, "resident_names": names[:5], "source": source}
...
async def _scrape_one_site(site: Dict[str, Any], street: str, city: str, state: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"phones": [], "emails": [], "resident_names": [], "source": site["name"], "error": str(e)}
...
async def trace_by_address(
    ...
    *,
    ...
    mobile_count = sum(1 for p in phones if p.get("line_type") in ("mobile", None, "unknown")
