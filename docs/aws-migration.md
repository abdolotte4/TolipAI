# AWS Migration Guide — TolipAI Scraper Engine
...
docker build -f Dockerfile.lambda -t TolipAI-scraper-lambda .

# Tag and push
docker tag TolipAI-scraper-lambda:latest \
  <ACCOUNT_ID>.dkr.ecr.us-east-1.amazonaws.com/TolipAI-scraper:latest
docker push <ACCOUNT_ID>.dkr.ecr.us-east-1.amazonaws.com/TolipAI-scraper:latest
```
...
|---|---|---|---|---|
| `TolipAI-health` | `workers.lambda_handler.health_handler` | 128 MB | 5s | Health probe |
| `TolipAI-distressed` | `workers.lambda_handler.distressed_handler` | 512 MB | 30s | Lis pendens / foreclosure search |
| `TolipAI-satellite` | `workers.lambda_handler.satellite_handler` | 2048 MB | 120s | Drive-for-dollars AI |
| `TolipAI-cash-buyers` | `workers.lambda_handler.cash_buyers_handler` | 1024 MB | 60s | Cash buyer discovery |
| `TolipAI-skip-trace` | `workers.lambda_handler.skip_trace_handler` | 512 MB | 30s | OSINT skip trace |
| `TolipAI-propwire` | `workers.lambda_handler.handler` | 3008 MB | 300s | Propwire comps / history |
| `TolipAI-router` | `workers.lambda_handler.handler` | 1024 MB | 120s | Universal router (dev/test) |

### Step 3: Wire to API Gateway (HTTP API)

```
POST /{proxy+}  →  TolipAI-router Lambda
```
...
Set `S3_BUCKET=TolipAI-scraper-results` on each Lambda. Results auto-save to:
```
s3://TolipAI-scraper-results/distressed/YYYY/MM/DD/HHMMSS_<job_id>.json
s3://TolipAI-scraper-results/satellite/YYYY/MM/DD/HHMMSS_<job_id>.json
```
...
Point Athena at `s3://TolipAI-scraper-results/` to run ad-hoc queries:
```sql
...
Set `USE_REKOGNITION=1` on the `TolipAI-satellite` Lambda.
...
- Log groups: `/aws/lambda/TolipAI-*`
- Dashboard: scrapes/min, error rate, p99 duration

### Secrets Manager
Replace env vars with Secrets Manager references:
```
TolipAI/google-maps-key     → GOOGLE_MAPS_API_KEY
TolipAI/attom-api-key       → ATTOM_API_KEY
TolipAI/proxy-credentials   → PROXY_URL
TolipAI/openrouter-key      → OPENROUTER_API_KEY
```

### Lambda env var pattern:
```python
import boto3
secret = boto3.client("secretsmanager").get_secret_value(SecretId="TolipAI/google-maps-key")
...
response = textract.analyze_document(
    Document={"S3Object": {"Bucket": "TolipAI-raw-docs", "Name": "deed.pdf"}},
