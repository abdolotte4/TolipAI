...
# Fetch current inline policy (will fail gracefully if it doesn't exist yet)
...
if [ -z "$EXISTING_POLICY" ]; then
  ...
  # Check if secretsmanager:GetSecretValue is already granted on TolipAI/scraper/*
...
sys.exit(1)
" 2>/dev/null; then
    echo "    ✓ secretsmanager:GetSecretValue already granted on TolipAI/scraper/* — no change needed."
    POLICY_ACTION="skip"
  else
...
if [ "$POLICY_ACTION" != "skip" ]; then
  # Build a minimal policy granting GetSecretValue on all TolipAI/scraper/* secrets
  # (suffixed ARN wildcard — the '*' matches the random 6-char suffix appended by Secrets Manager)
...
# Remove any stale secretsmanager statements to avoid duplicates
doc["Statement"] = [
